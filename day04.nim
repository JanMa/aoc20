...
proc valid_2(p: Pass): int =
  result = 1
  if p.byr < 1920 or p.byr > 2002: return 0
  if p.iyr < 2010 or p.iyr > 2020: return 0
  if p.eyr < 2020 or p.eyr > 2030: return 0
  if p.hgt.len > 2:
  ...
  if p.hcl.len > 0 and p.hcl[0] == '#':
    try: discard parseHexInt(p.hcl)
    except: return 0
  else: return 0
  if p.ecl notin ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]:
    return 0
  if p.pid.len == 9:
    try: discard parseInt(p.pid)
    except: return 0
  else: return 0
